          ...
          if(text.replace(/\s/g, '').length){
            ...
            if(text.includes("<")){
              if(text.split("<")[0].replace(/\s/g, '').length){
                ...
                addVar(`${from}/${filename}`, id, pretext)
                size += 1
              } 
              if(text.substr(text.lastIndexOf('>')+1, text.length).replace(/\s/g, '').length > 1) {
                console.log('something at the end')
                /* let endtext = text.substr(text.lastIndexOf('>'), text.length)
                let id = endtext.substr(0, 12).replace(/\s/g, '_').replace(`'`, '_').replace(',', '_').replace("(","").replace(")", '_') + size
                let spanEnd = `<span id="${id}">${endText}</span>`
                text = text.replace(endText, spanEnd)
                ...
                let newPiece = piece.replace(text, spanText)
                html1 = html1.replace(piece, newPiece);
                addVar(`${from}/${filename}`, id, posttext)
                size += 1
              } 
            }
            else {
